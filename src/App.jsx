...
const PRODUCT_CATEGORIES = ["Haircut","Beard Trim","Shave","Hair Color","Hair Treatment","Product Sale","Kids Cut","Style/Blowout","Other Service"];
const EXPENSE_CATEGORIES = ["Rent/Booth Fee","Supplies","Equipment","Marketing","Insurance","Licensing","Utilities","Product Inventory","Continuing Education","Other"];
const PAYMENT_METHODS = ["Cash", "Card", "CashApp", "Zelle", "Other"];

let _idCounter = 0;
function generateId() { _idCounter++; return Date.now().toString(36) + _idCounter.toString(36) + Math.random().toString(36).slice(2, 7); }

function getWeekNumber(d) { const date = new Date(d); date.setHours(0,0,0,0); date.setDate(date.getDate()+3-((date.getDay()+6)%7)); const week1 = new Date(date.getFullYear(),0,4); return Math.round(((date-week1)/86400000-3+((week1.getDay()+6)%7))/7)+1; }
...
function getWeekRange(dateStr) {
  ...
  return { start: monday.toLocaleDateString("en-US",{month:"short",day:"numeric"}), end: sunday.toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}) };
}
...
function generateSampleData() {
  ...
  const notes=["Regular","Walk-in","Referral from Dre","First time","VIP client","","",""];
  const income=[],expenses=[];
  const startDate=new Date(2026,1,1),endDate=new Date(2026,2,15);
  for(let d=new Date(startDate);d<=endDate;d.setDate(d.getDate()+1)){
    ...
    for(let c=0;c<numClients;c++){
      ...
      const tip = Math.random()>0.3 ? [5,7,10,15,20][Math.floor(Math.random()*5)] : 0;
      const payment = ["Cash","Card","CashApp","Card","Cash","Card"][Math.floor(Math.random()*6)];
      income.push({id:generateId(),date:dateStr,category,amount,tip,payment,client:clients[Math.floor(Math.random()*clients.length)],note:notes[Math.floor(Math.random()*notes.length)]});
    }
  }
  [1,2].forEach(m=>{
    const mStr=`2026-${String(m+1).padStart(2,"0")}`;
    expenses.push({id:generateId(),date:`${mStr}-01`,category:"Rent/Booth Fee",amount:600,note:`${m===1?"Feb":"Mar"} booth rent`,client:"",tip:0,payment:""});
    expenses.push({id:generateId(),date:`${mStr}-01`,category:"Insurance",amount:85,note:"Liability insurance",client:"",tip:0,payment:""});
    expenses.push({id:generateId(),date:`${mStr}-05`,category:"Utilities",amount:45,note:"Shop utilities share",client:"",tip:0,payment:""});
  });
  for(let d=new Date(startDate);d<=endDate;d.setDate(d.getDate()+7)){
    const dateStr=d.toISOString().split("T")[0];
    expenses.push({id:generateId(),date:dateStr,category:"Supplies",amount:[25,32,40,28,35][Math.floor(Math.random()*5)],note:"Blades, capes, towels",client:"",tip:0,payment:""});
    if(Math.random()>0.5) expenses.push({id:generateId(),date:dateStr,category:"Product Inventory",amount:[45,60,80,55][Math.floor(Math.random()*4)],note:"Pomade & oils restock",client:"",tip:0,payment:""});
  }
  expenses.push({id:generateId(),date:"2026-02-10",category:"Equipment",amount:189,note:"New Wahl clippers",client:"",tip:0,payment:""});
  expenses.push({id:generateId(),date:"2026-02-18",category:"Marketing",amount:50,note:"Instagram boost",client:"",tip:0,payment:""});
  expenses.push({id:generateId(),date:"2026-03-05",category:"Marketing",amount:75,note:"Business cards & flyers",client:"",tip:0,payment:""});
  expenses.push({id:generateId(),date:"2026-03-10",category:"Continuing Education",amount:120,note:"Fade mastery workshop",client:"",tip:0,payment:""});
  return {incomeEntries:income,expenseEntries:expenses};
}

const initialState = {incomeEntries:[],expenseEntries:[]};
...
function Onboarding({onDone}){
  ...
  const steps=[
    ...
    {icon:"💰",title:"Log Sales, Tips & Expenses",desc:"Add each service with tips and payment method (cash, card, CashApp). Categorize everything with one tap."},
    {icon:"📊",title:"Weekly & Monthly P&L",desc:"Automatically generated Profit & Loss reports break down your revenue, tips, costs, and net profit at a glance."},
  ...
  return (
    <div style={{background:C.bg,minHeight:"100vh",fontFamily:fb,color:C.text,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24,textAlign:"center"}}>
      ...
      <button onClick={()=>last?onDone():setStep(step+1)} style={{padding:"16px 48px",borderRadius:12,border:"none",cursor:"pointer",fontSize:16,fontWeight:700,fontFamily:fb,background:C.accent,color:C.bg}}>{last?"Let's Go!":"Next"}</button>
...
export default function BarberPnL(){
  ...
  useEffect(()=>{
    Promise.all([loadData(),loadOnboarded()]).then(([d,ob])=>{
      ...
      setOnboarded(!!ob); setLoaded(true);
    });
  },[]);
  ...
  function handleLoadDemo(){
    ...
    dispatch({type:"LOAD",payload:demo});
    showToast("📊 Demo data loaded! Check your reports.");
    setView("monthly");
  }
  ...
  function handleSubmit(){
    ...
    const tip=parseFloat(formTip)||0;
    const entry={id:generateId(),date:formDate,category:formCategory,amount:amt,tip,payment:formPayment,note:formNote,client:formClient};
    dispatch({type:entryType==="income"?"ADD_INCOME":"ADD_EXPENSE",payload:entry});
    setFormAmount("");setFormTip("");setFormNote("");setFormClient("");
    const totalMsg=tip>0?` + ${formatCurrency(tip)} tip`:"";
    showToast(entryType==="income"?`💰 ${formatCurrency(amt)}${totalMsg} added!`:"📝 Expense added!");
  }
  ...
  // ── Daily Summary ──
  const todayEntries=state.incomeEntries.filter(e=>e.date===today);
  const todayRevenue=todayEntries.reduce((s,e)=>s+e.amount,0);
  const todayTips=todayEntries.reduce((s,e)=>s+(e.tip||0),0);
  const todayClients=todayEntries.length;
  const todayCash=todayEntries.filter(e=>e.payment==="Cash").reduce((s,e)=>s+e.amount+(e.tip||0),0);
  const todayCard=todayEntries.filter(e=>e.payment==="Card").reduce((s,e)=>s+e.amount+(e.tip||0),0);
  const todayOther=todayEntries.filter(e=>e.payment!=="Cash"&&e.payment!=="Card").reduce((s,e)=>s+e.amount+(e.tip||0),0);

  // ── CSV Export ──
  function exportCSV(reportType){
    let csv="";
    if(reportType==="all"){
      csv="Type,Date,Category,Amount,Tip,Payment,Client,Note\n";
      state.incomeEntries.forEach(e=>{csv+=`Income,${e.date},"${e.category}",${e.amount.toFixed(2)},${(e.tip||0).toFixed(2)},${e.payment||""},"${e.client||""}","${e.note||""}"\n`;});
      state.expenseEntries.forEach(e=>{csv+=`Expense,${e.date},"${e.category}",${e.amount.toFixed(2)},0.00,,"","${e.note||""}"\n`;});
    }else if(reportType==="weekly"){
      csv="Week,Week Range,Revenue,Tips,Total Income,Expenses,Net Profit,Margin %\n";
      buildWeeklyData().forEach(([key,d])=>{const p=d.income+d.tips-d.expenses;const mg=(d.income+d.tips)>0?((p/(d.income+d.tips))*100).toFixed(1):"0.0";const r=getWeekRange(d.dateExample);csv+=`${key},"${r.start} - ${r.end}",${d.income.toFixed(2)},${d.tips.toFixed(2)},${(d.income+d.tips).toFixed(2)},${d.expenses.toFixed(2)},${p.toFixed(2)},${mg}\n`;});
    }else{
      csv="Month,Revenue,Tips,Total Income,Expenses,Net Profit,Margin %\n";
      buildMonthlyData().forEach(([key,d])=>{const p=d.income+d.tips-d.expenses;const mg=(d.income+d.tips)>0?((p/(d.income+d.tips))*100).toFixed(1):"0.0";const[yr,mo]=key.split("-");csv+=`"${new Date(+yr,+mo-1).toLocaleDateString("en-US",{month:"long",year:"numeric"})}",${d.income.toFixed(2)},${d.tips.toFixed(2)},${(d.income+d.tips).toFixed(2)},${d.expenses.toFixed(2)},${p.toFixed(2)},${mg}\n`;});
  ...
  // ── P&L calculations (now include tips) ──
  function buildWeeklyData(){
    ...
    [...state.incomeEntries,...state.expenseEntries].forEach(e=>{
      ...
      if(!weeks[key])weeks[key]={income:0,tips:0,expenses:0,incomeByCategory:{},expenseByCategory:{},dateExample:e.date,cashTotal:0,cardTotal:0};
      const isInc=state.incomeEntries.some(ie=>ie.id===e.id);
      if(isInc){weeks[key].income+=e.amount;weeks[key].tips+=(e.tip||0);weeks[key].incomeByCategory[e.category]=(weeks[key].incomeByCategory[e.category]||0)+e.amount;if(e.payment==="Cash")weeks[key].cashTotal+=e.amount+(e.tip||0);if(e.payment==="Card")weeks[key].cardTotal+=e.amount+(e.tip||0);}
  ...
  function buildMonthlyData(){
    ...
    [...state.incomeEntries,...state.expenseEntries].forEach(e=>{
      ...
      if(!months[key])months[key]={income:0,tips:0,expenses:0,incomeByCategory:{},expenseByCategory:{},cashTotal:0,cardTotal:0};
      const isInc=state.incomeEntries.some(ie=>ie.id===e.id);
      if(isInc){months[key].income+=e.amount;months[key].tips+=(e.tip||0);months[key].incomeByCategory[e.category]=(months[key].incomeByCategory[e.category]||0)+e.amount;if(e.payment==="Cash")months[key].cashTotal+=e.amount+(e.tip||0);if(e.payment==="Card")months[key].cardTotal+=e.amount+(e.tip||0);}
  ...
  const totalTips=state.incomeEntries.reduce((s,e)=>s+(e.tip||0),0);
  const totalExpenses=state.expenseEntries.reduce((s,e)=>s+e.amount,0);
  const netProfit=totalIncome+totalTips-totalExpenses;
  const hasData=state.incomeEntries.length>0||state.expenseEntries.length>0;
  ...
  const ExportBtn=({label,onClick})=><button onClick={onClick} style={{display:"inline-flex",alignItems:"center",gap:6,padding:"8px 14px",borderRadius:8,border:`1px solid ${C.border}`,background:C.card,color:C.accent,fontSize:12,fontWeight:600,fontFamily:fb,cursor:"pointer"}}><Icons.Download/>{label}</button>;
  ...
  return (
    <div style={{background:C.bg,minHeight:"100vh",fontFamily:fb,color:C.text,position:"relative"}}>
        ...
        <div style={{position:"fixed",inset:0,zIndex:900,display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
          ...
          <div style={{position:"relative",background:C.card,borderRadius:16,padding:24,width:"100%",maxWidth:360,border:`1px solid ${C.border}`}}>
            ...
            <div style={{borderTop:`1px solid ${C.border}`,paddingTop:20,marginBottom:16}}>
              <div style={{fontSize:11,color:C.textMuted,fontWeight:600,textTransform:"uppercase",letterSpacing:"0.05em",marginBottom:10}}>Demo</div>
              <button onClick={()=>{handleLoadDemo();setShowSettings(false);}} style={{display:"flex",alignItems:"center",gap:8,width:"100%",padding:"12px 16px",borderRadius:10,border:`1px solid ${C.border}`,background:C.bg,color:C.blue,fontSize:14,fontWeight:600,fontFamily:fb,cursor:"pointer"}}>
                <Icons.Zap/> Load Sample Data (see how it works)
              </button>
            </div>
            <div style={{borderTop:`1px solid ${C.border}`,paddingTop:20}}>
              ...
              {!confirmReset?
                <button onClick={()=>setConfirmReset(true)} style={{display:"flex",alignItems:"center",gap:8,width:"100%",padding:"12px 16px",borderRadius:10,border:`1px solid ${C.border}`,background:C.bg,color:C.red,fontSize:14,fontWeight:600,fontFamily:fb,cursor:"pointer"}}><Icons.Reset/> Clear All Data & Start Fresh</button>
              :
                <div style={{background:C.redDim,borderRadius:10,padding:16,border:"1px solid rgba(248,113,113,0.3)"}}>
                  <p style={{fontSize:13,color:C.red,margin:"0 0 12px",fontWeight:500}}>This will permanently delete all entries. Cannot be undone.</p>
      ...
      <div style={{padding:"28px 24px 20px",borderBottom:`1px solid ${C.border}`,background:`linear-gradient(180deg, #131715 0%, ${C.bg} 100%)`,display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        ...
        <button onClick={()=>setShowSettings(true)} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:10,cursor:"pointer",color:C.textMuted,marginTop:4}}><Icons.Gear/></button>
      </div>

      {/* Summary Cards */}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:8,padding:"16px 16px 8px"}}>
        {[
          {l:"Services",v:totalIncome,c:C.green},
          {l:"Tips",v:totalTips,c:C.blue},
          {l:"Expenses",v:totalExpenses,c:C.red},
          {l:"Net Profit",v:netProfit,c:netProfit>=0?C.green:C.red},
        ].map((x,i)=>(
          <div key={i} style={{background:C.card,borderRadius:12,padding:"12px 10px",border:`1px solid ${C.border}`,position:"relative",overflow:"hidden"}}>
            <div style={{position:"absolute",top:0,left:0,right:0,height:3,background:x.c,opacity:0.6}}/>
            <div style={{fontSize:9,color:C.textMuted,fontWeight:600,textTransform:"uppercase",letterSpacing:"0.05em",marginBottom:4}}>{x.l}</div>
            <div style={{fontSize:15,fontWeight:700,fontFamily:fd,color:x.c}}>{formatCurrency(x.v)}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{display:"flex",gap:4,padding:"12px 16px",borderBottom:`1px solid ${C.border}`}}>
        {[{id:"entry",label:"Add Entry",icon:<Icons.Plus/>},{id:"weekly",label:"Weekly",icon:<Icons.Calendar/>},{id:"monthly",label:"Monthly",icon:<Icons.Chart/>}].map(tab=>(
      ...
      <div style={{padding:16,maxWidth:700,margin:"0 auto"}}>
        ...
        {view==="entry"&&(
          <div>
            {/* ── Daily Summary Card ── */}
            {todayClients>0&&(
              <div style={{background:`linear-gradient(135deg, ${C.card} 0%, #1a1f1c 100%)`,borderRadius:14,padding:18,marginBottom:16,border:`1px solid ${C.border}`,position:"relative",overflow:"hidden"}}>
                <div style={{position:"absolute",top:0,left:0,right:0,height:3,background:`linear-gradient(90deg, ${C.green}, ${C.blue}, ${C.accent})`}}/>
                <div style={{fontSize:11,color:C.accent,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:12}}>Today's Summary</div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:12}}>
                  <div><div style={{fontSize:22,fontWeight:700,fontFamily:fd,color:C.green}}>{formatCurrency(todayRevenue)}</div><div style={{fontSize:10,color:C.textMuted,fontWeight:600}}>SERVICES</div></div>
                  <div><div style={{fontSize:22,fontWeight:700,fontFamily:fd,color:C.blue}}>{formatCurrency(todayTips)}</div><div style={{fontSize:10,color:C.textMuted,fontWeight:600}}>TIPS</div></div>
                  <div><div style={{fontSize:22,fontWeight:700,fontFamily:fd,color:C.accent}}>{todayClients}</div><div style={{fontSize:10,color:C.textMuted,fontWeight:600}}>CLIENTS</div></div>
                </div>
                {(todayCash>0||todayCard>0)&&(
                  <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                    {todayCash>0&&<span style={{fontSize:11,padding:"3px 8px",borderRadius:6,background:C.greenDim,color:C.green,fontWeight:600}}>Cash: {formatCurrency(todayCash)}</span>}
                    {todayCard>0&&<span style={{fontSize:11,padding:"3px 8px",borderRadius:6,background:C.blueDim,color:C.blue,fontWeight:600}}>Card: {formatCurrency(todayCard)}</span>}
                    {todayOther>0&&<span style={{fontSize:11,padding:"3px 8px",borderRadius:6,background:"rgba(200,169,110,0.12)",color:C.accent,fontWeight:600}}>Other: {formatCurrency(todayOther)}</span>}
                  </div>
                )}
              </div>
            )}
            ...
            {!hasData&&todayClients===0&&(
              <div style={{textAlign:"center",padding:"24px 16px",marginBottom:16,background:C.card,borderRadius:14,border:`1px solid ${C.border}`}}>
                <div style={{fontSize:36,marginBottom:12}}>✂️</div>
                <div style={{fontSize:15,fontWeight:600,color:C.text,marginBottom:6}}>Ready to track your money</div>
                <div style={{fontSize:13,color:C.textMuted,marginBottom:16}}>Add your first service below, or load sample data to see how reports look.</div>
                <button onClick={handleLoadDemo} style={{display:"inline-flex",alignItems:"center",gap:8,padding:"10px 20px",borderRadius:10,border:`1px solid ${C.accent}`,background:"transparent",color:C.accent,fontSize:14,fontWeight:600,fontFamily:fb,cursor:"pointer"}}>
                  <Icons.Zap/> Load Demo Data
                </button>
              </div>
            )}
            ...
            <div style={{background:C.card,borderRadius:14,padding:20,border:`1px solid ${C.border}`,marginBottom:20}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
                <div><label style={lbl}>Date</label><input type="date" value={formDate} onChange={e=>setFormDate(e.target.value)} style={inp}/></div>
                <div><label style={lbl}>Amount ($)</label><input type="number" step="0.01" min="0" placeholder="0.00" value={formAmount} onChange={e=>setFormAmount(e.target.value)} style={{...inp,color:C.accent,fontSize:18,fontWeight:700,fontFamily:fd}}/></div>
              </div>

              {entryType==="income"&&(
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
                  <div><label style={lbl}>Tip ($)</label><input type="number" step="0.01" min="0" placeholder="0.00" value={formTip} onChange={e=>setFormTip(e.target.value)} style={{...inp,color:C.blue,fontWeight:600}}/></div>
                  <div>
                    <label style={lbl}>Payment</label>
                    <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                      {PAYMENT_METHODS.map(p=>(
                        <button key={p} onClick={()=>setFormPayment(p)} style={{padding:"8px 10px",borderRadius:8,border:`1px solid ${formPayment===p?C.accent:C.border}`,background:formPayment===p?C.accent:"transparent",color:formPayment===p?C.bg:C.textMuted,fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:fb}}>
                          {p}
                        </button>
                      ))}
                    </div>
            ...
            <h3 style={{fontFamily:fd,fontSize:18,fontWeight:600,marginBottom:12,color:C.accent,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              ...
              {hasData&&<ExportBtn label="CSV" onClick={()=>exportCSV("all")}/>}
            </h3>
              ...
              .sort((a,b)=>b.date.localeCompare(a.date)||b.id.localeCompare(a.id)).slice(0,20).map(entry=>(
                <div key={entry.id} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 14px",background:C.card,borderRadius:10,marginBottom:6,border:`1px solid ${C.border}`}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:13,fontWeight:600,color:C.text}}>{entry.category}{entry.tip>0&&<span style={{color:C.blue,fontSize:11,marginLeft:6}}>+{formatCurrency(entry.tip)} tip</span>}</div>
            ...
            {!hasData&&<div style={{textAlign:"center",padding:20,color:C.textMuted,fontSize:14}}>No entries yet.</div>}
          </div>
        )}
        ...
        {view==="weekly"&&(
          <div>
            ...
            {buildWeeklyData().map(([wk,data])=>{
              const totalInc=data.income+data.tips; const p=totalInc-data.expenses; const r=getWeekRange(data.dateExample);
              const mg=totalInc>0?((p/totalInc)*100).toFixed(1):"0.0";
              return (
                <div key={wk} style={{background:C.card,borderRadius:14,padding:18,marginBottom:12,border:`1px solid ${C.border}`}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:8,marginBottom:14}}>
                    {[{l:"Services",v:data.income,c:C.green},{l:"Tips",v:data.tips,c:C.blue},{l:"Expenses",v:data.expenses,c:C.red},{l:"Net Profit",v:p,c:p>=0?C.green:C.red}].map(x=>(
                      <div key={x.l}><div style={{fontSize:10,color:C.textMuted,textTransform:"uppercase",fontWeight:600,letterSpacing:"0.05em"}}>{x.l}</div><div style={{fontSize:15,fontWeight:700,fontFamily:fd,color:x.c}}>{formatCurrency(x.v)}</div></div>
                    ))}
                  </div>
                  {(data.cashTotal>0||data.cardTotal>0)&&<div style={{display:"flex",gap:8,marginBottom:10}}>
                    {data.cashTotal>0&&<span style={{fontSize:11,padding:"3px 8px",borderRadius:6,background:C.greenDim,color:C.green,fontWeight:600}}>Cash: {formatCurrency(data.cashTotal)}</span>}
                    {data.cardTotal>0&&<span style={{fontSize:11,padding:"3px 8px",borderRadius:6,background:C.blueDim,color:C.blue,fontWeight:600}}>Card: {formatCurrency(data.cardTotal)}</span>}
                  </div>}
        ...
        {view==="monthly"&&(
          <div>
            ...
            {buildMonthlyData().length>1&&(()=>{
              ...
              const iv=sorted.map(([,d])=>d.income+d.tips);const ev=sorted.map(([,d])=>d.expenses);
              const mx=Math.max(...iv,...ev,1);
              return <div style={{background:C.card,borderRadius:14,padding:18,marginBottom:12,border:`1px solid ${C.border}`}}>
                ...
                <div style={{display:"flex",gap:16}}>
                  <div style={{flex:1}}><MiniBar data={iv} maxVal={mx} color={C.green}/><div style={{fontSize:10,color:C.green,marginTop:4,textAlign:"center",fontWeight:600}}>Income</div></div>
            ...
            {buildMonthlyData().map(([mk,data])=>{
              const totalInc=data.income+data.tips;const p=totalInc-data.expenses;const[yr,mo]=mk.split("-");
              ...
              return (
                <div key={mk} style={{background:C.card,borderRadius:14,padding:18,marginBottom:12,border:`1px solid ${C.border}`}}>
                  ...
                  <div style={{background:C.bg,borderRadius:10,padding:14,border:`1px solid ${C.border}`}}>
                    ...
                    {data.tips>0&&<div style={{display:"flex",justifyContent:"space-between",padding:"3px 8px",fontSize:13}}><span style={{color:C.blue}}>Tips</span><span style={{color:C.blue,fontWeight:500}}>{formatCurrency(data.tips)}</span></div>}
                    <div style={{display:"flex",justifyContent:"space-between",padding:"6px 8px",fontSize:14,fontWeight:700,borderTop:`1px solid ${C.border}`,marginTop:4}}><span style={{color:C.text}}>Total Income</span><span style={{color:C.green}}>{formatCurrency(totalInc)}</span></div>
                    {(data.cashTotal>0||data.cardTotal>0)&&<div style={{display:"flex",gap:8,padding:"6px 8px"}}>
                      {data.cashTotal>0&&<span style={{fontSize:11,padding:"2px 6px",borderRadius:4,background:C.greenDim,color:C.green,fontWeight:600}}>Cash: {formatCurrency(data.cashTotal)}</span>}
                      {data.cardTotal>0&&<span style={{fontSize:11,padding:"2px 6px",borderRadius:4,background:C.blueDim,color:C.blue,fontWeight:600}}>Card: {formatCurrency(data.cardTotal)}</span>}
                    </div>}
